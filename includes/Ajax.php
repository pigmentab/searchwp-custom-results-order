...
class Ajax {
	...
	public function clear_buoys() {
		...
		if ( ! SWP()->is_valid_engine( $engine ) || empty( $query ) ) {
			wp_send_json_error();
		}
	...
	public function promote_result() {
		...
		if ( empty( $post_id ) || ! SWP()->is_valid_engine( $engine ) || empty( $query ) ) {
			wp_send_json_error();
		}
	...
	public function get_search_results() {
		...
		if ( ! SWP()->is_valid_engine( $engine ) || empty( $query ) || ! class_exists( 'SWP_Query' ) ) {
			wp_send_json_error();
		}
